-- Complete the failed upgrade - bypass trigger approach
-- First disable the trigger, then do manual inserts, then re-enable

-- Temporarily disable the trigger
ALTER TABLE public.subscription_events DISABLE TRIGGER ALL;

-- 1. Add the missing subscription event for the upgrade (without triggering sync)
INSERT INTO public.subscription_events (
  ...
  'cs_upgrade_manual_fix_20250903',
  'Side-Gig',
  'Full-Time',
  15,
  jsonb_build_object(
...
-- 4. Manually update the subscriptions table to Full-Time
...
-- 5. Update profiles table to reflect Full-Time plan and add tokens
...
ALTER TABLE public.subscription_events ENABLE TRIGGER ALL;