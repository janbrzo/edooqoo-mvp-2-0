...
export const downloadCurrentViewAsHtml = async (elementId: string, filename: string, isTeacherVersion: boolean, title: string) => {
  try {
    ...
    const noPdfElements = elementClone.querySelectorAll('[data-no-pdf="true"]');
    noPdfElements.forEach(el => el.remove());
    
    // Remove all teacher tips sections when generating student version
    if (!isTeacherVersion) {
      const teacherTips = elementClone.querySelectorAll('.teacher-tip');
      teacherTips.forEach(el => el.remove());
    }
    
    // Create a new document structure
    const htmlDocContent = document.createElement('html');
    ...
    htmlDocContent.appendChild(body);
    
    // Serialize the document
    const htmlContent = '<!DOCTYPE html>\n' + htmlDocContent.outerHTML;
