...
const RatingSection: React.FC<RatingSectionProps> = ({
  ...
  return <div data-no-pdf="true" className="p-6 rounded-lg mt-10 mb-6 text-center bg-white">
      ...
      <div className="flex justify-center space-x-2 mb-2 rounded-none bg-transparent">
        {[1, 2, 3, 4, 5].map(star => <button key={star} onClick={() => handleStarClick(star)} onMouseEnter={() => setHover(star)} onMouseLeave={() => setHover(0)} className="focus:outline-none transition-transform transform hover:scale-110" aria-label={`Rate ${star} stars`}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-md" data-no-pdf="true">
          ...
          <div className="flex justify-center mt-3 mb-4">
            {[1, 2, 3, 4, 5].map(star => <Star key={star} size={38} strokeWidth={1.3} className={rating >= star ? "text-yellow-400 fill-yellow-400" : "text-gray-300"} />)}
          </div>
          ...
          <div className="flex justify-end space-x-2 mt-2">
            ...
            <Button size="sm" variant="default" onClick={handleSubmitRating} className="bg-[#3d348b] text-white hover:bg-[#3d348b]/90">
              Submit Feedback
            </Button>
          </div>
        </DialogContent>
